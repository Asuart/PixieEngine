...
RenderPipeline* RenderEngine::GetPipeline() {
	return s_pipeline;
}
...
MeshHandle RenderEngine::LoadMesh(const Mesh& mesh) {
	return s_renderer->LoadMesh(mesh);
}

TextureHandle RenderEngine::LoadTexture(const Buffer2D<float>& texture) {
	return s_renderer->LoadTexture(texture);
}

TextureHandle RenderEngine::LoadTexture(const Buffer2D<glm::vec3>& texture) {
	return s_renderer->LoadTexture(texture);
}

TextureHandle RenderEngine::LoadTexture(const Buffer2D<glm::vec4>& texture) {
	return s_renderer->LoadTexture(texture);
}
...
//
//void RenderEngine::DrawTextureFitted(TextureHandle texture, glm::ivec2 textureResolution, glm::ivec2 viewportResolution) {
...
//}
//
//void RenderEngine::DrawTexture(TextureHandle id, glm::vec2 pos, glm::vec2 size) {
//	m_quadShader.Bind();
