...
void main() {        
    vec3 mAlbedo = texture(gAlbedo, fTexCoord).rgb;
    float mSpecular = texture(gSpecular, fTexCoord).r;     
    vec3 mWorldPos = texture(gPosition, fTexCoord).rgb;
    float mRoughness = texture(gRoughness, fTexCoord).a;
    vec3 mNormal = texture(gNormal, fTexCoord).rgb;
    float mMetallic = texture(gMetallic, fTexCoord).a;
